...
//  options.damping = 1e-1;
  ...
  for(uint i=1; i<=initialPath.d0 - horizon; ++i){
    ...
    if(i<=1){
      komo.setConfiguration(-2, smoothed[0]);
      komo.setConfiguration(-1, smoothed[0]);
    }else{
      komo.setConfiguration(-2, smoothed[i-2]);
      komo.setConfiguration(-1, smoothed[i-1]);
    }
    //-- initialize other configurations with the previous horizon
    for(uint j=0; j<horizon; ++j){
      komo.setConfiguration(j, smoothed[i+j]);
    }
